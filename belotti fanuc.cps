/**
  ...
  BELOTTI/FANUC post processor configuration.

  $Revision: 44083 865c6f1c385b9194ab63e73899f0a4787fce12a6 $
  $Date: 2023-08-14 12:16:17 $

  FORKID {E960D641-2997-4485-B216-B525867FB2FB}
*/

description = "Belotti FLU/FLA/SKY - Fanuc";
vendor = "Belotti cnc machining centers";
vendorUrl = "http://www.belotti.com";
legal = "Copyright (C) 2012-2023 by Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 45917;

longDescription = "Milling post for Belotti FLU/FLA/SKY - Fanuc";
...
properties = {
  ...
  aAxisLimits: {
    title      : "A Axis angular limits",
    description: "Specifies angular limits for A Axis",
    group      : "configuration",
    type       : "integer",
    values     : [
      {id:110, title:"-110deg +110deg"},
      {id:120, title:"-120deg +120deg"},
      {id:135, title:"-135deg +135deg"}
    ],
    value: 110,
    scope: "post"
  },
...
var mFormat = createFormat({prefix:"M", zeropad:false, decimals:1});
...
var mClampModal = createModalGroup({strict:false}, [[110, 111], [112, 113]], mFormat); // 4th axis lock/unlock, 5th axis lock/unlock
...
function defineMachine() {
  ...
  if (true) { // note: setup your machine here
    var aAxis = createAxis({coordinate:0, table:false, axis:[1, 0, 0], range:[-getProperty("aAxisLimits"), getProperty("aAxisLimits")], tcp:useTCP});
    var cAxis = createAxis({coordinate:2, table:false, axis:[0, 0, 1], range:[-270, 270], tcp:useTCP});
  ...
  if (!receivedMachineConfiguration) {
    ...
    machineConfiguration.setRetractPlane(toPreciseUnit(-150, MM)); // home position Z
  }
}
...
function onOpen() {
  ...
  var saveUseG95 = getProperty("useG95");
  setProperty("useG95", false); // FPM is required for the initial retracts
  writeRetract(Z);
  gMotionModal.reset();
  forceFeed();
  writeBlock(gFormat.format(53), gMotionModal.format(1), feedOutput.format(10000), aOutput.format(0), cOutput.format(0));
  forceABC();
  setProperty("useG95", saveUseG95);

  writeBlock(gFeedModeModal.format(getProperty("useG95") ? 95 : 94));
}
...
function disableLengthCompensation(force) {
  if (lengthCompensationActive || force) {
    ...
    writeBlock(gFormat.format(49) + " H0");
  ...
  if (insertToolCall || newWorkOffset || newWorkPlane || smoothing.cancel) {
    ...
    if (!isFirstSection()) {
      ...
      if (insertToolCall) {
        gMotionModal.reset();
        forceFeed();
        writeBlock(gFormat.format(53), gMotionModal.format(1), getFeed(10000), aOutput.format(0), cOutput.format(0));
        forceABC();
      }
    }
  }
...
function onRewindMachineEntry(_a, _b, _c) {
  if (getProperty("safeRetractDistance") <= 0) {
    error(localize("Please set the property -safeRetractDistance- to a positive value for a safe retraction."));
  }
  return false;
}
...
function onCommand(command) {
  ...
  case COMMAND_LOCK_MULTI_AXIS:
    ...
    if (machineConfiguration.isMultiAxisConfiguration() && (machineConfiguration.getNumberOfAxes() >= 4)) {
      code = mClampModal.format(110);
      writeBlock(conditional(code, code + SP + formatComment("CLAMP C"))); // lock 4th-axis motion
      if (machineConfiguration.getNumberOfAxes() == 5) {
        code = mClampModal.format(112);
        writeBlock(conditional(code, code + SP + formatComment("CLAMP A"))); // lock 5th-axis motion
      }
    }
    return;
  case COMMAND_UNLOCK_MULTI_AXIS:
    var code;
    if (machineConfiguration.isMultiAxisConfiguration() && (machineConfiguration.getNumberOfAxes() >= 4)) {
      code = mClampModal.format(111);
      writeBlock(conditional(code, code + SP + formatComment("UNCLAMP C"))); // unlock 4th-axis motion
      if (machineConfiguration.getNumberOfAxes() == 5) {
        code = mClampModal.format(113);
        writeBlock(conditional(code, code + SP + formatComment("UNCLAMP A"))); // unlock 5th-axis motion
      }
    }
    return;
...
function writeRetract() {
  ...
  if (retract && retract.words.length > 0) {
    ...
    case "G53":
      ...
      writeBlock(gAbsIncModal.format(90), gFormat.format(53), gMotionModal.format(1), getFeed(10000), retract.words);
      break;
    default:
